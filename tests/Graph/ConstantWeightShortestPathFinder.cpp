...
TEST_CASE("ConstantWeightShortestPathFinder", "[graph]") {
  ...
  SECTION("path of length 4") {
    ...
    std::vector<std::string> expect = {"1", "2", "3", "4"};
    for (size_t i = 0; i < result.length(); i++) {
      auto vert = result.vertexToAqlValue(spo->cache(), i);
//      CHECK(vert.slice().get(StaticStrings::KeyString).isEqualString(expect.at(i)));
    ...
    //   std::cout << edge.slice().get(StaticStrings::FromString).toString() << std::endl;
    //   // isEqualString(expect.at(i)));
    // }
  }

  SECTION("two paths of length 4") {
    std::cout << "two paths" << std::endl;
  ...
  std::cout << "delete finder" << std::endl;
  delete finder;
